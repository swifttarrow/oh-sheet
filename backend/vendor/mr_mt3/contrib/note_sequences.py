...
from . import event_codec, vocabularies, run_length_encoding
import note_seq
            ...
            if value.is_drum:
                ...
                if state is not None:
                    state.active_pitches[(
                        value.pitch, value.program)] = velocity_bin
    ...
    if event.type == 'pitch':
        ...
        if state.is_tie_section:
            ...
            if (pitch, state.current_program) not in state.active_pitches:
                # Model thinks this note is ringing but we don't have it tracked.
                # Start a fresh note at the segment boundary instead of dropping it.
                state.active_pitches[(pitch, state.current_program)] = (
                    time, DEFAULT_VELOCITY)
