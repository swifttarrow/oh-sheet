...
def decode_events(
    ...
    for token_idx, token in enumerate(tokens):
        try:
            ...
            continue
        if event.type == 'shift':
            # If we're still in the tie section when we see a shift, the tie
            # section is over — auto-close it. The model may omit the explicit
            # 'tie' token when there are no notes to tie.
            if hasattr(state, 'is_tie_section') and state.is_tie_section:
                try:
                    from . import event_codec as _ec
                    decode_event_fn(state, cur_time, _ec.Event('tie', 0), codec)
                except ValueError:
                    pass
